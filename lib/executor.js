...
module.exports = function execute(app, instructions, callback) {
  ...
  // Setup the components and run the boot scripts in series synchronously or asynchronously
  // Please note async supports both styles
  async.series([
    function(done) {
      setupComponents(app, instructions, done);
    },
    function(done) {
...
function setupComponents(app, instructions, callback) {
  async.eachSeries(instructions.components, function(data, done) {
    ...
    data.config = getUpdatedConfigObject(app, data.config, opts);
    if (configFn.length >= 3) {
      debug('Starting async function %j', data.sourceFile);
      configFn(app, data.config, function(err) {
        debug('Async function finished %j', data.sourceFile);
        done(err);
      });
    } else {
      debug('Starting sync function %s', data.sourceFile);
      var error;
      try {
        configFn(app, data.config);
        debug('Sync function finished %s', data.sourceFile);
      } catch (err) {
        debug('Sync function failed %s', data.sourceFile, err);
        error = err;
      }
      done(error);
    }
