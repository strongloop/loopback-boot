...
function defineMixins(app, instructions) {
  ...
  if (modelBuilder.mixins && mixinNames.length > 0) {
    modelBuilder.mixins.meta = {};
    mixinNames.forEach(function(name) {
      var meta = mixins[name];
      var mixin = require(meta.filepath);
      if (typeof mixin === 'function' || mixin.prototype instanceof BaseClass) {
        debug('Configuring mixin %s', meta.name || name);
        modelBuilder.mixins.define(name, mixin); // TODO (name, mixin, meta)
        modelBuilder.mixins.meta[name] = meta;
      } else {
        debug('Skipping mixin file %s - `module.exports` is not a function' +
          ' or Loopback model', mixins[name]);
      }
    });
  }
}
