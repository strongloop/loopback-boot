...
module.exports = function compile(options) {
  ...
  if (options.models && options.models['__extend'] === true){
    modelsConfig = _.extend(ConfigLoader.loadModels(modelsRootDir, env), modelsConfig);
    delete modelsConfig["__extend"];
  }
  ...
  var modelDefinitions = options.modelDefinitions  || [];
...
function buildAllModelInstructions(rootDir, modelsConfig, sources, modelDefinitions) {
  var registry = findModelDefinitions(rootDir, sources, modelDefinitions);
...
function loadAdhocModelDefinitions(rootDir, modelDefinitions) {
  var registry = {};

  if (modelDefinitions.definitions) {
    // if a master source file dir is set, load it.
    var modelDefSrcDir = (modelDefinitions.sourceDir) ? resolveSourceDir(rootDir, modelDefinitions.sourceDir) : undefined;
    var files = (!modelDefSrcDir) ? [] : tryReadDir(modelDefSrcDir);

    modelDefinitions.definitions.forEach(function(modelDef) {
      var entry = loadModelDefinition(modelDefSrcDir || rootDir, modelDef, files);
      var modelName = entry.definition.name;
      if (!modelName) {
        debug('Skipping ad-hoc model definition without Model name: %s',
...
function findModelDefinitions(rootDir, sources, modelDefinitions) {
  var registry = loadAdhocModelDefinitions(rootDir, modelDefinitions);
...
/**
 * Loads a module definition from a file or a ad-hoc object.
 * @param rootDir
 * @param jsonFile path to file or an object with 'definition' property and optional 'sourceFile' property.
 * @param allFiles
 * @returns {{definition: *, sourceFile: *}}
 */
function loadModelDefinition(rootDir, jsonFile, allFiles) {
  ...
  if (typeof jsonFile === "string") {
    definition =  require(jsonFile);
    basename = path.basename(jsonFile, path.extname(jsonFile));
    sourceDir = path.dirname(jsonFile);
  }
  else {
    definition =  jsonFile.definition;
    basename = definition.name.toLowerCase(); //TODO: dasherize the name since "testModel" will be "test-model.js" (find the imp in SL code)
    if (jsonFile.sourceFile) {
      sourceFile = resolveSourceDir(rootDir, jsonFile.sourceFile);
  ...
  debug('Found model "%s" - %s %s', definition.name,
    (typeof jsonFile === "string") ? path.relative(rootDir, jsonFile) : 'ad-hoc',
