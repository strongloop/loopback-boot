...
function tryReadDir() {
  var bootDirectory = 'boot';
  var sourceDirectory = arguments[0].split('/').pop();

  function read(params) {
    return fs.readdirSync.apply(fs, params);
  }

  function subDirFinder(path) {
    var sufix = path.split('/'+bootDirectory+'/').pop();

    if (fs.statSync(path).isDirectory()) {
      var subDirFiles = read([path]);
      _.forEach(subDirFiles, function (item) {
        subDirFinder(path+'/'+item);
      });
    } else {
      files.push(sufix);
    }
  ...
  try {
    var files = read(arguments);

    if (sourceDirectory === bootDirectory) {
      subDirFinder(path);
    }
