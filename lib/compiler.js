  ...
  phasesNames.forEach(function(phase) {
    ...
    Object.keys(phaseConfig).forEach(function(middleware) {
      ...
      allConfigs.forEach(function(config) {
        ...
        // resolved.sourceFile will be `false` if an optional middleware is not resolvable
        // if a non-optional middleware is not resolvable, it will throw at resolveAppPath()
        if (resolved.sourceFile) {
          ...
          debug('Middleware "%s" not found: %s', middleware, resolved.optional)
        }
...
function resolveMiddlewarePath(rootDir, middleware, config) {
  ...
  if (!fragment) {
    ...
    resolved.optional = config.optional
    return resolved;
  }
